...
remote points to the original (`Hmbown/CodeWhale`).

The routine flow is: fetch both remotes, `git merge upstream/main` inside the
submodule, resolve conflicts, verify both workspaces build, then commit the
submodule first and update the submodule pointer in picus. The non-obvious
parts that broke things before are below—read them carefully.

### Cargo.toml conflict resolution (`crates/*/Cargo.toml`)

Conflicts are always in the same pattern: the fork uses `.workspace = true`
for internal `codewhale-*` deps, upstream uses `path`+`version`. Resolution:

1. **Keep `.workspace = true`** for every `codewhale-*` internal dependency.
   New upstream crates that an existing crate gains a dep on (e.g. `hooks`
   gained `codewhale-release`) also get `.workspace = true`.
2. **Hardcode `version`/`license`/`repository`** in each `[package]` section
   (do NOT use `*.workspace = true`) because picus's workspace values differ
   from CodeWhale's. Bump the version to match the new upstream release.
3. **`Cargo.lock`**: take upstream's (`git checkout --theirs Cargo.lock`).

### Update both root Cargo.toml files

**This is the step that was missed the first time and broke standalone
builds of `codewhale-cli` and `codewhale-tui`.**

When upstream adds a new `codewhale-*` crate, you must add it to
`workspace.dependencies` in **both** root manifests:

- **CodeWhale root** (`thirdparty/CodeWhale/Cargo.toml`): add
  `codewhale-foo = { path = "./crates/foo" }`. Without this, standalone builds
  inside `thirdparty/CodeWhale/` fail because `.workspace = true` has nothing
  to resolve against.
- **picus root** (`Cargo.toml`): add the crate to `members` and add
  `codewhale-foo = { path = "thirdparty/CodeWhale/crates/foo" }` to
  `workspace.dependencies`. Also add any new external dep the crate needs
  (e.g. `rustls` for `codewhale-release`) with versions from the CodeWhale
  root manifest.

If upstream changed feature sets for shared deps (e.g. `tokio`, `reqwest`),
check that picus's unified definition still satisfies both; picus keeps
`tokio = ["full"]` and `reqwest` with the union of features, not upstream's
narrower set.

### Verify both workspaces

```
# picus (from repo root)
cargo check --workspace && cargo test -p example_picuscode
# CodeWhale standalone (from thirdparty/CodeWhale/)
cargo check -p codewhale-cli && cargo check -p codewhale-tui
```

Both must pass. Commit inside the submodule and push to its `origin` first,
then commit the picus submodule-pointer update (plus `Cargo.toml`/`Cargo.lock`)
and push to picus's `origin`.
