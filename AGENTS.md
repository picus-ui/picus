...
(`codewhale-*` v0.8.68). The fork's own `thirdparty/CodeWhale/Cargo.toml`
...
## 1.2. Synchronizing the CodeWhale upstream

`thirdparty/CodeWhale` is a git submodule tracking a fork of CodeWhale. Its
`origin` remote points to the fork (`Summpot/CodeWhale`) and its `upstream`
remote points to the original (`Hmbown/CodeWhale`). The following procedure
synchronizes upstream changes into the fork while keeping picus and the fork
both buildable. Every step below was learned the hard way—skip none.

### Step 1 — Fetch and assess the divergence

```
cd thirdparty/CodeWhale
git fetch origin
git fetch upstream
git rev-list --count HEAD..upstream/main   # commits upstream is ahead
git rev-list --count upstream/main..HEAD   # commits fork is ahead
```

If upstream is ahead, a merge is needed. If the fork also has its own commits
(non-zero `upstream/main..HEAD`), the merge will produce conflicts in Cargo.toml
files—this is expected.

### Step 2 — Merge and resolve Cargo.toml conflicts

```
git merge upstream/main
```

Conflicts will appear in `crates/*/Cargo.toml`. The fork changed internal
`codewhale-*` dependencies from `path`+`version` to `.workspace = true` so they
resolve against the picus root. Upstream keeps the `path`+`version` form. The
resolution is always the same:

1. **Keep `.workspace = true`** for every `codewhale-*` internal dependency.
   Do not accept the upstream `path`+`version` form, or picus workspace
   inheritance breaks.
2. **New upstream crates**: if upstream adds a new `codewhale-*` crate and an
   existing crate gains a dependency on it (e.g. `hooks` gained
   `codewhale-release`), also write it as `.workspace = true`.
3. **Version numbers**: bump the hardcoded `version = "0.8.xx"` in every
   affected `crates/*/Cargo.toml` `[package]` section to match the new
   `workspace.package.version`. These are hardcoded (not
   `version.workspace = true`) because picus's `workspace.package.version` is
   `0.1.0`, not the CodeWhale version. Also hardcode `license = "MIT"` and
   `repository = "https://github.com/Hmbown/CodeWhale"` (not
   `*.workspace = true`) for the same reason—picus uses different values.
4. **`Cargo.lock`**: take the upstream version (`git checkout --theirs
   Cargo.lock && git add Cargo.lock`). It is regenerated by cargo anyway and is
   not used by the picus workspace.

### Step 3 — Update the CodeWhale root Cargo.toml

**This is the step that was missed the first time and broke standalone builds.**

The CodeWhale root `thirdparty/CodeWhale/Cargo.toml` has its own
`[workspace.dependencies]` section listing `codewhale-* = { path = "./crates/..." }`
entries. When a new `codewhale-*` crate appears (like `codewhale-release`), you
must add it here too:

```toml
codewhale-release = { path = "./crates/release" }
```

Without this entry, `codewhale-release.workspace = true` in `hooks` resolves
 when building inside the picus workspace (picus defines it), but **fails**
 when building `codewhale-cli` or `codewhale-tui` standalone inside
`thirdparty/CodeWhale/` because the CodeWhale root does not define it.

Also check whether upstream added new external dependencies to
`[workspace.dependencies]` (e.g. `rustls`, `axum`, `clap`) and decide whether
picus needs them too.

### Step 4 — Update the picus root Cargo.toml

1. **New crate members**: add any new `codewhale-*` crate that is transitively
   needed by an existing picus workspace member to the `members` list. For
   example, `codewhale-release` was added because `codewhale-hooks` (already a
   member) now depends on it.
2. **New workspace.dependencies entries**: add the corresponding
   `codewhale-release = { path = "thirdparty/CodeWhale/crates/release" }` to
   picus's `[workspace.dependencies]`.
3. **New external dependencies**: if the new crate needs an external dep not
   already in picus's `[workspace.dependencies]` (e.g. `rustls` for
   `codewhale-release`), add it there with the version and features from the
   CodeWhale root manifest.
4. **Dep version drift**: if upstream changed feature sets for shared deps
   (e.g. `tokio` from `["full"]` to a granular list, or `reqwest` feature
   changes), check whether picus's unified definition still satisfies both
   picus and CodeWhale. Picus keeps `tokio = ["full"]` and `reqwest` with the
   union of features needed by both; do not blindly copy upstream's narrower
   set.

### Step 5 — Verify both workspaces

```
# picus workspace (from repo root)
cargo check --workspace
cargo test -p example_picuscode

# CodeWhale standalone (from thirdparty/CodeWhale/)
cargo check -p codewhale-cli
cargo check -p codewhale-tui
cargo check              # default-members: cli, app-server, tui
```

Both must pass before committing. The picus check ensures `.workspace = true`
resolution works; the CodeWhale standalone check ensures the fork root
`workspace.dependencies` is complete.

### Step 6 — Commit and push

Commit inside the submodule first, then update the submodule pointer in picus:

```
# In thirdparty/CodeWhale/
git add -A
git commit -m "Merge upstream/main (vX.Y.Z) into main ..."
git push origin main

# In picus root
git add Cargo.toml Cargo.lock thirdparty/CodeWhale
git commit -m "merge: bump CodeWhale submodule to vX.Y.Z upstream ..."
