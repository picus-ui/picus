...
pub fn spawn_i18n_page(commands: &mut Commands, parent: Entity) -> Entity {
    ...
    commands.spawn((
        UiLabel::new("Select a locale to switch the active Fluent bundle and font stack:"),
        ChildOf(locale_card),
    ));
    ...
    let demo_row = commands
        .spawn((UiFlexRow, class("gallery.card"), ChildOf(locale_card)))
        .id();
    commands.spawn((UiLabel::new("Localized greeting:"), ChildOf(demo_row)));
    commands.spawn((
        LocalizeText::new("gallery-hello"),
    ...
    commands.spawn((
        LocalizeText::new("gallery-current-locale"),
        class("gallery.note"),
        ChildOf(locale_card),
    ));
    ...
    commands.spawn((
        UiLabel::new("The following text is resolved from Fluent bundles by key:"),
        ChildOf(trans_card),
    ));

    let key_demo = commands
        .spawn((UiFlexColumn, class("gallery.card"), ChildOf(trans_card)))
        .id();
    commands.spawn((
        LocalizeText::new("gallery-han-unification"),
        ChildOf(key_demo),
    ));
    commands.spawn((
        LocalizeText::new("gallery-locale-action"),
        ChildOf(key_demo),
    ));
    commands.spawn((UiButton::new("Demo Button (static)"), ChildOf(key_demo)));
